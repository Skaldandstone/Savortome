...
# Shopping-list loading resilience (2026-09-24)

- Shopping-list contents and optional cart-provider choices now load independently. A provider-status failure cannot discard a successfully loaded list, and a list failure cannot masquerade as an empty list.
- Initial and refresh failures have separate retry controls and retained-data wording. Shopping-list writes keep their existing independent error state, so an unavailable provider lookup does not block checking, removing, or clearing visible items.
- Focused actual-component coverage proves independent list/provider success, failure and retry behavior, retained list contents after a failed refresh, and absence of the false `Nothing on the list` state after loading fails.
- Rendered the actual woodland list presentation with a populated local fixture and failed provider lookup. At 390 x 844 with 200% text, the document had no horizontal overflow (`innerWidth=390`, `scrollWidth=375`), the retry control exceeded 44 px, and every remove control measured 44 x 44 px. At the default 1280 px desktop viewport and 100% text, retry and remove controls remained 44 px. The list remained readable after adding a two-row large-text layout for amount and name. A temporary local review route was removed before the production build. This does not prove signed-in list data, provider behavior, device behavior, grocery ordering, or owner acceptance.

# Weekly-plan loading resilience (2026-09-24)

- A failed week request no longer renders an empty planner grid or enabled add controls. It states that the saved plan has not changed and offers a dedicated retry while keeping write errors separate.
- Retrying the currently loaded week retains its last visible plan with explicit stale wording. Changing weeks clears the prior week from view until the selected week loads, and cancellation guards prevent a late prior-week response from replacing the current selection.
- Focused actual-component coverage proves failure does not create an empty planner, retry restores all 21 meal slots, and an older deferred response cannot replace the newly selected week.
- Rendered the actual woodland planner against a local failed week request. At 390 x 844 with 200% text, the document had no horizontal overflow (`innerWidth=390`, `scrollWidth=375`), the retry target exceeded 44 px, and no meal-add controls were present. At the default 1280 px desktop viewport and 100% text, the retry target was 44 px and add controls remained absent. The failure copy, navigation, disabled shopping-list action, brass boundary, and responsive stacking were visually inspected. A temporary local review route was removed before the production build. This does not prove signed-in plan data, successful writes, device behavior, deployment, or owner acceptance.

# Saved-allergen check resilience (2026-09-24)

- Recipe and cook surfaces now distinguish checking, checked-with-no-match, detected conflict, and unavailable states. A rejected dietary-profile read cannot silently look like a conflict-free recipe.
- The unavailable state says that no saved-allergen comparison was made, instructs the reader to treat ingredients as unchecked, and provides a retry that performs only the profile read.
- Focused actual-component coverage proves the failure boundary hides the conflict-free state and a successful retry restores the expected allergen warning without a write.
- Rendered the actual woodland warning against a local signed-out response. At 390 x 844 with 200% text, the document had no horizontal overflow (`innerWidth=390`, `scrollWidth=375`) and the retry target was over 100 px tall. At 1280 px with 100% text, the retry target was 44 px and the warning stayed within its 680 px content column. The dark-theme border, copy wrapping, navigation and footer were visually inspected. A temporary local review route was removed before the production build. This does not prove a signed-in profile response, a real allergen determination, assistive-technology traversal, device behavior, deployment, or owner acceptance.

# Discover loading resilience (2026-09-24)

- An initial Discover failure no longer renders the same empty-community message as a successful empty response or offers a web-search fallback based on missing data. The page states that no empty result was assumed and provides a retry.
- Failed refreshes retain the last successfully loaded cards with explicit stale wording. Request versioning prevents a slow initial load or older search from replacing a newer query.
- Focused actual-component and controller coverage proves the false-empty boundary, retry wiring and stale-response protection without performing a write.
- Rendered the actual woodland Discover failure view. At 390 x 844 with 200% text, the document had no horizontal overflow (`innerWidth=390`, `scrollWidth=375`), the retry target exceeded 60 px, and neither empty-community copy nor fallback results appeared. At 1280 px with 100% text, the retry target was 44 px and the false-empty copy remained absent. The dark-theme panel, form, callout wrapping, navigation and footer were visually inspected. A temporary local review route was removed before the production build. This does not prove live shared-recipe data, a signed-in search, assistive-technology traversal, deployment, device behavior, or owner acceptance.
