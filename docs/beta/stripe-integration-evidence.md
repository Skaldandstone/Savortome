...
The Stripe account/sandbox identity, catalog, authenticated portal browser flow, hosted webhook delivery, RDS locks, refunds and historical reconciliation remain unverified. No real charges, purchases, Stripe objects, portal configuration, keys, secret injection or live settings were created. Keep `STRIPE_CHECKOUT_ENABLED=false` and `STRIPE_LIVEMODE=false`. Portal plan changes remain disabled until subscription tiers are mapped from actual Prices. Tax registration/configuration needs separate review before any paid release; this work did not enable automatic tax.
