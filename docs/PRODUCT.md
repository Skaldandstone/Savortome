...
| Blog without it | Page prose â†’ Claude | One call |
| YouTube | Caption track (+ description) â†’ Claude | One call |
| TikTok | Post caption â†’ Claude | One call |
| Instagram / Facebook | Caption via yt-dlp â†’ Claude | One call |
| Video whose captions we can't fetch directly | yt-dlp â†’ caption track â†’ Claude | One call |
| Video with no captions at all | yt-dlp â†’ audio â†’ ASR â†’ Claude | One call + ASR |
...
test video renders "Jacques PÃ©pin" as "zck Pepa".)
...
**What it's worth**, measured on the same Jacques PÃ©pin video:

| | Without yt-dlp | With it |
|---|---|---|
| Path | description â†’ Claude | transcript â†’ Claude |
...
make right now. Neither answers the ordinary question â€” *where did I put that
recipe?* â€” which is what the library search box does.
...
there is also Aâ€“Z, quickest, best rated, and recently cooked. Nulls always sort
last, whichever direction the column runs â€” a recipe with no stated time is the
...
never showed it back to you. Cards now carry your stars and "Cooked 3 times" â€”
...
two â€” a sort link that silently dropped your search would be worse than no
sort link.
...
`/plan` is a week of meals â€” seven days, three slots each, every gap visible.
...
- **Dates are `date` columns and ISO `YYYY-MM-DD` strings**, never timestamps
  ...
  for the same reason â€” the daylight-saving Sunday that has twenty-five hours
  in it is a real week someone will plan, and it's a test.
- **The key is `(user, date, slot, recipe)`.** Several recipes in one slot is
  the normal case â€” a main and a side â€” while the same recipe twice in one
...
| Rows | Meaning |
|---|---|
| `A â†’ B pending` | A has asked B. Only the requester's row exists. |
| `A â†’ B accepted` + `B â†’ A accepted` | They're friends. |
| `A â†’ B blocked` | A has blocked B - and B sees no relationship at all, rather than a rejection. |
...
One wrinkle worth knowing if you touch the search column: `to_tsvector('english', â€¦)`
...
Every recipe can carry per-serving nutrition â€” calories, protein, carbs, fat,
fiber, sodium â€” built from one of three sources, in a strict order of trust:
...
awake â€” Wake Lock on the web, `expo-keep-awake` on the phone â€” because
...
- **They're anchored to the wall clock**, not counted down on a tick. A tick
  drifts, and stops entirely when the tab is backgrounded or the phone sleeps â€”
...
- **Steps typed by hand get timers too.** The same deterministic
  ...
  "simmer for 20 minutes" is all anyone has to do â€” and changing the number in
  the text moves the timer with it.
- **â–¶ Watch this bit** deep-links into the source video at that second, for
  recipes that came from one.

Ticking the last step marks the recipe cooked â€” bumping `timesCooked`, which
pantry search and discovery both lean on â€” and asks for a rating there and
...
swapped out, or killed â€” which is the whole reason anyone sets a cook timer.
