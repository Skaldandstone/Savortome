...
function CareContent({ link, standalone = false, accountId }: CareProps & { accountId: string | null }) {
  ...
  return <main className={styles.care}>
    <section className={styles.intro} aria-labelledby="care-title">
      <div><p className={styles.eyebrow}>A little care, at your pace</p><h1 id="care-title">Feed me gently</h1><p>Something easy is a good place to start.<br />Let’s find what feels possible right now.</p></div>
      <img className={styles.scene} src="/woodland/hearth.png" alt="" />
    </section>
    <section className={styles.choices} aria-label="Optional food choices">
      <label>How much preparation?<select value={choices.effort ?? ''} onChange={e => setChoice('effort', e.target.value || undefined)}><option value="">Anything manageable</option>{CARE_EFFORTS.map(v => <option key={v} value={v}>{CARE_EFFORT_LABELS[v]}</option>)}</select></label>
      <label>How much time?<select value={choices.time ?? ''} onChange={e => setChoice('time', e.target.value || undefined)}><option value="">Up to twenty minutes</option><option value="two">Two minutes</option><option value="ten">Ten minutes</option><option value="twenty">Twenty minutes</option></select></label>
      <details className={styles.moreChoices}><summary>A few more preferences</summary><div className={styles.choices}>
    ...
    <section className={styles.results} aria-label="Food suggestions" aria-live="polite">
      ...
      {results.map(({ kind, label, food, pantryMatches }, index) => <article className={styles.card} key={food.id} data-kind={kind} data-selected={selectedId === food.id}>
        <div className={styles.cardTop}><span className={styles.number}>0{index + 1}</span><p className={styles.eyebrow}>{label}</p><span className={styles.time}>{food.minutes} min</span></div>
        <h2>{food.title}</h2>{selectedId === food.id && <p className={styles.selected}>Selected idea</p>}<p>{food.description}</p>
        ...
        {kind !== 'future' ? <ol>{food.steps.map(step => <li key={step}>{step}</li>)}</ol> : <p>A small thing to have on hand, whenever you next shop.</p>}
        <p className={styles.note}>The list action adds {food.shoppingItem} only. Check any other ingredients you need.</p>
        <button className={kind === 'future' ? styles.secondary : styles.action} aria-label={`Add ${food.shoppingItem} to my list for ${food.title}`} disabled={busy} onClick={() => void add(food)}>{busy && selectedId === food.id ? 'Adding...' : `Add ${food.shoppingItem} to my list`}</button>
      </article>)}
    </section>
    {message && <div className={styles.feedback} role="status"><p>{message}</p>{signInNeeded && <a href={careSignInHref(handoff)} onClick={saveReturn}>Sign in and return</a>}</div>}
    <img className="woodland-prop" src="/woodland/kettle-journal.png" alt="" /><p className={styles.disclaimer}>{CARE_DISCLAIMER}</p>
    <footer className={styles.footer}><span>No perfect meals required.</span>{handoff.return_to && <a href={handoff.return_to}>Back to Wispling</a>}<a href="/cook">Back to cooking</a></footer>
  </main>;
}
