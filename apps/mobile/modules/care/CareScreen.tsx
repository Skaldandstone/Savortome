...
import { CareIdeaCard } from './CareIdeaCard';
...
function CareContent({ accountId }: { accountId: string | null }) {
  ...
  return <ScrollView style={{ backgroundColor: c.bg }} contentContainerStyle={{ padding: 20, paddingTop: insets.top + 16, paddingBottom: insets.bottom + 32, maxWidth:760, width:'100%',alignSelf:'center' }}>
    ...
    <View style={{paddingBottom:18,borderBottomWidth:1,borderColor:c.border,marginBottom:12}}>
      <Text style={{color:c.textMuted,fontSize:12,letterSpacing:1.2}}>A LITTLE CARE, AT YOUR PACE</Text>
      <Text accessibilityRole="header" style={[styles.title,{color:c.accent}]}>Feed me gently</Text>
      <Text style={[styles.body,{color:c.textMuted}]}>Something easy is a good place to start.</Text>
    </View>
    ...
    {results.map(({label,food,pantryMatches}) => <CareIdeaCard key={food.id} label={label} food={food} pantryMatches={pantryMatches} usePantry={!!choices.usePantry} selected={selectedId === food.id} busy={busy} onAdd={() => void add(food)} />)}
