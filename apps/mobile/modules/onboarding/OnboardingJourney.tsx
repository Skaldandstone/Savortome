...
export function CookingStep({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  const [profile, setProfile] = useState<CookProfile>({});
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const requestVersion = useRef(0);

  useEffect(() => { let active = true; const version = ++requestVersion.current; setProfile({}); setLoading(true); setSaving(false); setError(""); void client.cookingProfile().then(value => { if (active && requestVersion.current === version) setProfile(value); }).catch(() => { if (active && requestVersion.current === version) setError("Your cooking preferences could not load. You can skip this and try later."); }).finally(() => { if (active && requestVersion.current === version) setLoading(false); }); return () => { active = false; requestVersion.current += 1; }; }, [client]);
