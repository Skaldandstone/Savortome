...
[CmdletBinding()]
param(
  [ValidateSet('Inspect','Prepare','Deploy','Rollback')][string]$Mode='Inspect',
  [string]$Image,
  [string[]]$ClerkUserIds=@(),
  [switch]$DisableBeta,
...
$profile_='secondbreakfast-toolkit'; $region_='us-east-2'
$cluster_='secondbreakfast-cluster'; $service_='secondbreakfast-web'
...
function VerifyCodeBuildReport([string]$Reference,[string]$Commit,$Remote) {
  ...
  if($CodeBuildId -cnotmatch '^secondbreakfast-web-build:[a-f0-9-]{36}$' -or -not $BuildArtifactZip){throw 'Supply an existing trusted CodeBuild ID and its downloaded ZIP artifact.'}
  ...
  if($build.id -cne $CodeBuildId -or $build.projectName -cne 'secondbreakfast-web-build' -or $build.arn -cne "arn:aws:codebuild:us-east-2:574921529762:build/$CodeBuildId" -or $build.buildStatus -cne 'SUCCEEDED'){throw 'CodeBuild identity or successful status could not be verified.'}
  $sha=[Security.Cryptography.SHA256]::Create()
  ...
  if($build.source.type -ceq 'S3'){
    ...
    if($build.source.location -cnotmatch '^secondbreakfast-build-source-574921529762/(.+)$'){throw 'Unexpected source bucket.'}
    $key=$Matches[1]
    $head=AwsJson -Arguments @('s3api','head-object','--bucket','secondbreakfast-build-source-574921529762','--key',$key,'--version-id',$build.sourceVersion,'--checksum-mode','ENABLED','--expected-bucket-owner','574921529762')
...
if($identity.Account -ne '574921529762'){throw 'Wrong AWS account. Refusing release.'}
...
if($Mode -eq 'Prepare') {
  ...
  if($Image -and $Image -cnotmatch '^574921529762\.dkr\.ecr\.us-east-2\.amazonaws\.com/[a-z0-9/_-]+@sha256:[a-f0-9]{64}$'){throw 'Image must use a digest in the existing AWS account and region.'}
  if(-not $DisableBeta -and -not $Image){throw 'A reviewed immutable image is required.'}
  $remote=@{codeBuildId=$CodeBuildId;buildArtifactZip=$BuildArtifactZip;reviewedSourceSha256=$ReviewedSourceSha256;reviewedBuildspecSha256=$ReviewedBuildspecSha256}
