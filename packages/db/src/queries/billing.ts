...
/** Call inside a transaction before reading Stripe state or changing billing. */
export async function lockBillingUser(database: Database, userId: string) {
...
export async function stripePurchaseFulfilled(database: Database, sessionId: string): Promise<boolean> {
  const row = await database.query.creditPurchases.findFirst({
    where: eq(schema.creditPurchases.stripeSessionId, sessionId), columns: { fulfilledAt: true },
  });
  ...
  return database.transaction(async (transaction) => {
    ...
    const [purchase] = await transaction
      .select({ id: schema.creditPurchases.id, fulfilledAt: schema.creditPurchases.fulfilledAt,
        userId: schema.creditPurchases.userId, productId: schema.creditPurchases.productId,
        cents: schema.creditPurchases.cents, credits: schema.creditPurchases.credits, tier: schema.creditPurchases.tier })
    ...
    if (purchase.userId !== record.userId || purchase.productId !== record.productId
      || purchase.cents !== record.cents || purchase.credits !== record.credits || purchase.tier !== record.tier) {
      throw new Error("Existing Stripe purchase does not match its owner or product; reconciliation required.");
    }
    if (purchase.fulfilledAt !== null) return null;
 ...
 *
 * Legacy callers clear the stored id when dropping to free. Verified webhook
 * callers retain it through unpaid/paused states, and match that exact identity
 * under the billing account lock before reading the current Stripe state.
 */
export async function applySubscriptionTier(
  ...
  await database
    .update(schema.users)
    .set({ tier, stripeSubscriptionId: tier === "free" && !options.retainSubscription ? null : subscriptionId })
    .where(eq(schema.users.id, userId));
